...
class ContextSelectorDialog(QtGui.QDialog):
    ...
    def __init__(self, app, message, defaults=None, parent=None):
        ...
        self.shot_template_selector = shotgun_search_widget.GlobalSearchWidget(self)
        self.shot_template_selector.set_searchable_entity_types({
            'TaskTemplate': [['entity_type', 'is', 'Shot']],
        })
        self.shot_template_selector.set_bg_task_manager(self._task_manager)
        self.shot_template_selector.completer().entity_activated.disconnect(self.shot_template_selector.clear)
        ...
        if self.hide_parent_template_selector:
            self.template_selector.hide()
            self.new_tab_layout.labelForField(self.template_selector).hide()
    ...
    def default_template(self):
        return self.app.get_setting('shot_parent_task_template')

    @property
    def default_shot_template(self):
        return self.app.get_setting('task_template')

    @property
    def default_entity_type(self):
        return self.app.get_setting('shot_parent_entity_type')

    @property
    def supported_entity_types(self):
        return sorted(set([self.default_entity_type]))
    ...
    def set_shot_task_template(self, template):
        """
        Set the task_template option using a ShotGrid TaskTemplate or code. 
        ...
        if isinstance(template, dict):
            self.shot_template_selector.setText(template.get('name', template.get('code', '')))
            self._shot_template = template
            return
        else:
            template = self.app.shotgun.find_one(
                'TaskTemplate',
                [['code', 'is', template], ['entity_type', 'is', 'Shot']],
                ['code', 'entity_type']
            )
            if template:
                self.shot_template_selector.setText(template['code'])
                self._shot_template = template
                return

        self.shot_template_selector.clear()
        self._shot_template = None
    ...
    def set_options(self, options):
        ...
        if options.get('shot_task_template'):
            self.set_shot_task_template(options['shot_task_template'])
        else:
